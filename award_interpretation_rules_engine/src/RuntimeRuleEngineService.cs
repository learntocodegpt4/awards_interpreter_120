...
public sealed class RuntimeRuleEngineService : IRuntimeRuleEngineService
{
    ...
    public async Task<RuleCalculationResult> CalculateAsync(RuleCalculationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        ApplyAggregateExportBlocking(result.Calculation, _options.Engine);
    ...
    private static void ApplyAggregateExportBlocking(AggregatePayRunResult aggregate, EngineOptions options)
    {
        if (!options.BlockPayrollExportOnErrors ||
            !aggregate.Warnings.Any(w => w.BlocksPayrollExport) ||
            aggregate.PayrollLines.Count == 0)
            return;

        aggregate.BlockedPayrollLines.AddRange(aggregate.PayrollLines.Select(l => l with
        {
            SourceBucket = "blocked_payroll_lines",
            Exportable = false,
            RequiresReview = true
        }));
