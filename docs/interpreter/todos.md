...
| INT-010 | IN_PROGRESS | Publish parser metrics and traceability: source row, clause, raw text, parser version, confidence, provenance. | Metrics and trace columns/events emitted per ETL run. | `RosteredAI_ETL/tests/test_clause_interpreter.py` asserts parser version, source hash, confidence, and evidence. | QA verifies a parsed pay-related field traces back to source row and clause text. |
| INT-011 | IN_PROGRESS | Build source text bundling across allowance, penalty, clause statement, table header, payment frequency, and rate unit fields. | Source bundle object passed to all parser modules. | `RosteredAI_ETL/tests/test_clause_interpreter.py` proves field provenance is retained per extracted value. | QA verifies a parsed value shows the exact source field and text. |
| INT-012 | IN_PROGRESS | Add compounding and base-rate-reference extraction for penalty wording. | `is_compounding`, `base_rate_reference`, and stacking hints in condition output. | `RosteredAI_ETL/tests/test_golden_corpus.py` covers `ordinary rate`, `loaded rate`, and ambiguous wording. | QA reviews compounding decisions for sample penalty rows. |
...
| INT-016 | IN_PROGRESS | Add penalty parser coverage equal to allowance parser coverage. | Penalty semantic extraction for day, time, compounding, stacking, and public holiday conditions. | Golden corpus tests for MA000120 overtime, weekend, and ambiguous penalty rows. | QA validates penalty rows from awards with weekday, weekend, overtime, and public holiday penalties. |
...
- Golden corpus fixture added in `RosteredAI_ETL/tests/fixtures/interpreter/ma000120_golden_rows.json`.
- Focused automated tests added in `RosteredAI_ETL/tests/test_clause_interpreter.py`.
- Golden corpus regression coverage added in `RosteredAI_ETL/tests/test_golden_corpus.py`.
