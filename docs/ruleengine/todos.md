...
| RULE-002 | TODO | Compile published award semantics into rule conditions consumed by the backend engine. | Compiler from published semantic rows to `rules_json`. | Compiler tests using golden interpreter outputs. | QA confirms compiled rules match sample source clauses. |
